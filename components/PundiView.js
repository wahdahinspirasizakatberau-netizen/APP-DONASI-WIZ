...
const PundiView = ({ pundis = [], setPundis, riwayatPundis = [], setRiwayatPundis, contacts = [], programs = [], user, syncDataToSheet, darkMode, setViewImage, amils = [], setActiveTab }) => {
    ...
    // Mengumpulkan opsi zona default + zona baru yang pernah diketik manual
    ...
    const handleGetCurrentLocationGPS = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setGpsMessage({ type: 'success', text: `Tautan GPS berhasil disalin: ${mapsUrl}` });
                setTimeout(() => setGpsMessage(null), 7000);
            },
            (error) => {
                setGpsLoading(false);
                let errText = "Gagal mengambil titik GPS lokasi.";
                if (error.code === 1) errText = "Izin akses lokasi GPS ditolak oleh browser/HP.";
                else if (error.code === 2) errText = "Posisi GPS tidak ditemukan. Pastikan GPS HP aktif.";
                else if (error.code === 3) errText = "Waktu permintaan GPS habis. Coba ulangi lagi.";
                setGpsMessage({ type: 'error', text: errText });
    ...
    const renderGpsHelperBanner = (isInsideModal = false) => (
        <div className={`p-3.5 sm:p-4 rounded-2xl border shadow-sm space-y-2 ${
            isInsideModal 
                ? 'bg-gradient-to-r from-emerald-50 via-teal-50/70 to-white dark:from-emerald-950/40 dark:via-gray-800 dark:to-gray-800 border-emerald-200 dark:border-emerald-800 mb-4' 
                : 'bg-gradient-to-r from-emerald-50 via-teal-50 to-white dark:from-emerald-950/40 dark:via-gray-800 dark:to-gray-800 border-emerald-200 dark:border-emerald-800'
        }`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h4 className="text-xs sm:text-sm font-black text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5">
                        <span className="w-6 h-6 rounded-lg bg-wiz-green text-white flex items-center justify-center text-[10px] shadow-sm">
                            <i className="fa-solid fa-location-crosshairs"></i>
                        </span>
                        Alat Bantu Titik Lokasi GPS & Google Maps
                    </h4>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">
                        Sedang berada di lokasi pundi? Klik tombol untuk mengambil titik koordinat GPS saat ini dan langsung menyalin tautannya untuk ditempel ke form.
                    </p>
                </div>
            ...
            {gpsMessage && (
                <div className={`p-2.5 rounded-xl text-xs font-semibold flex items-center gap-2 animate-in ${gpsMessage.type === 'success' ? 'bg-emerald-100/90 text-emerald-800 border border-emerald-300 dark:bg-emerald-900/60 dark:text-emerald-200 dark:border-emerald-700' : 'bg-red-50 text-red-600 border border-red-200 dark:bg-red-950/50 dark:text-red-300'}`}>
                    <i className={`fa-solid ${gpsMessage.type === 'success' ? 'fa-circle-check text-emerald-600' : 'fa-triangle-exclamation text-red-500'}`}></i>
                    <span className="truncate">{gpsMessage.text}</span>
                    {gpsMessage.type === 'success' && (
                        <span className="text-[10px] ml-auto bg-emerald-200 dark:bg-emerald-800 px-2 py-0.5 rounded font-bold shrink-0">Siap Ditempel</span>
                    )}
                </div>
            )}
        </div>
            ...
            {activeSubTab === 'master' && (
                <div className="space-y-4 animate-in">
                    ...
                    <ModuleView 
                        ...
                        modalTopContent={renderGpsHelperBanner(true)}
                        onSave={savePundi} 
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    ...
                                                    {p.zona && (
                                                        <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                                                            {p.zona}
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <tr key={p.id} className={`hover:bg-wiz-light dark:hover:bg-gray-700/50 ${isChecked ? 'bg-wiz-green/10 dark:bg-emerald-950/30' : tStatus === 'Berhasil' ? 'bg-wiz-green/5 dark:bg-emerald-900/10' : tStatus === 'Dijemput' ? 'bg-yellow-50 dark:bg-yellow-900/10' : ''}`}>
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {p.zona && (
                                                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800">
                                                            {p.zona}
                                                        </span>
                                                    )}
                                                </div>
