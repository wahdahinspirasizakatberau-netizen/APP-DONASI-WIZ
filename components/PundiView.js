        ...
        <div className="space-y-6 slide-up relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                    <Button 
                        ...
                        variant="secondary"
                        className="shadow-sm font-bold text-xs py-2 px-3 border-wiz-green/30 text-wiz-green hover:bg-wiz-green/10"
                        icon="fa-solid fa-bullseye"
                    >
                        🎯 Atur Target
                    </Button>

                    {/* Tombol Kamera diaktifkan di semua layar HP & Laptop */}
                    <Button 
                        onClick={() => setActiveTab('scanner')} 
                        variant="primary" 
                        className="shadow-md shadow-wiz-green/30 px-3.5 py-2 text-xs font-bold"
                    ...
                    {isAdmin ? (
                        <div className="flex items-center gap-2.5 bg-white dark:bg-gray-800 px-3 py-1.5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                            <div className="w-8 h-8 rounded-xl bg-wiz-green/10 text-wiz-green dark:text-emerald-400 flex items-center justify-center font-bold text-sm">
                                <i className="fa-solid fa-user-tag"></i>
                            </div>
                            <div>
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Tampilkan Amil</p>
                                <select
                                ...
                                >
                                    ...
                                    {allAmilNames.map((name, idx) => (
                                        <option key={idx} value={name} className="dark:bg-gray-800">{name}</option>
                                    ))}
                                </select>
                            </div>
                            {selectedAmilFilter !== 'Semua' && (
                                <button
                                    ...
                                    className="text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 p-1 rounded-lg ml-0.5"
                        ...
                        <div className="flex items-center gap-2.5 bg-wiz-green/10 dark:bg-emerald-900/30 border border-wiz-green/20 dark:border-emerald-800/50 px-3 py-1.5 rounded-2xl">
                            <div className="w-8 h-8 rounded-xl bg-wiz-green text-white flex items-center justify-center font-bold text-xs shadow-sm">
                                <i className="fa-solid fa-user-check"></i>
                            </div>
                            <div>
                                <p className="text-[9px] font-bold text-wiz-green_dark dark:text-emerald-300 uppercase tracking-wider">Amil</p>
                                <p className="text-xs font-black text-wiz-green dark:text-emerald-400">{user?.name}</p>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {activeSubTab === 'dashboard' && (
                <div className="space-y-6 animate-in">
                    ...
                    {/* Grid Kartu Nominal Terkumpul & Target */}
                ...
                <form onSubmit={saveTargetConfig} className="space-y-4">
                    <div className="p-3.5 bg-wiz-green/5 dark:bg-emerald-950/30 rounded-2xl border border-wiz-green/20">
                        <p className="text-xs text-wiz-green dark:text-emerald-400 font-semibold leading-relaxed">
                            <i className="fa-solid fa-bullseye mr-1.5"></i> Tentukan target penarikan <b>Harian, Bulanan, dan Tahunan</b> serta masa berlaku target.
                        </p>
                    </div>
            ...
            <Modal isOpen={isBelumDijemputModalOpen} onClose={() => setIsBelumDijemputModalOpen(false)} title={`Daftar Pundi Belum Dijemput (${stats.belumDijemputCount})`}>
                <div className="space-y-3">
                    <p className="text-xs text-gray-500">Daftar kotak pundi aktif yang belum ditarik pada periode bulan yang dipilih:</p>
