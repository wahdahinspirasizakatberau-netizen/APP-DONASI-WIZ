...
const App = () => {
    ...
    const syncDataToSheet = async (sheetName, newData, itemsToAdd = null) => {
        // Amankan data di cache lokal
        safeSetJSON('wiz_cache_' + sheetName, newData);
        try {
            const url = typeof API_URL !== 'undefined' ? API_URL : '';
            if (url) {
                // Kirim request ke Google Apps Script:
                // Jika ada itemsToAdd (data baru dari cetak checklist/nota), kirimkan juga action 'appendData'
                // dan tetap kirimkan data lengkap (newData) yang sudah digabung agar riwayat lama tidak hilang
                const payload = {
                    action: (itemsToAdd && itemsToAdd.length > 0) ? 'appendData' : 'syncData',
                    sheetName: sheetName,
                    data: (itemsToAdd && itemsToAdd.length > 0) ? itemsToAdd : newData,
                    fullData: newData
                };
